        ...
        if s[s_idx] == '0':
            ...
            if len(s) - s_idx < 5:
                ...
                if piece == '10001':
                    ...
                    raise ValueError("Error parsing position setup, piece: "+str(piece) + "@pos " + str(s_idx) + " from " + str(s))
                s_idx += 5
                b_idx += 1
...
def convert_pos_to_cb(position):
    ...
    return cb_position, \
        (w_queens, w_rooks, w_bishops, w_knights, w_kings, w_pawns), \
        (b_queens, b_rooks, b_bishops, b_knights, b_kings, b_pawns)
...
def decode_position(cbg_file, offset):
    ...
    # and piece lists
    cb_pos, w_lists, b_lists = convert_pos_to_cb(position)
    return FEN, position, cb_pos, w_lists, b_lists
...
# tkn         = encoding byte
def do_piece_move(piece_list, piece_type, piece_nr, cb_position, cb_enc_arr, node, tkn):
    (i, j) = piece_list[piece_nr]
    cb_position[i][j] = (0, None)
...
def do_w_pawn_move(piece_list, piece_nr, cb_position, cb_enc_arr, node, tkn):
    return do_piece_move(piece_list, W_PAWN, piece_nr, cb_position, cb_enc_arr, node, tkn)


def do_b_pawn_move(piece_list, piece_nr, cb_position, cb_enc_arr, node, tkn):
    (i, j) = piece_list[piece_nr]
    #print("pawn at: "+str(i) + " "+str(j))
    cb_position[i][j] = (0, None)
    (add_x, add_y) = cb_enc_arr[tkn]
    add_x = -add_x  # revert direction as we pawn moves are always encoded
    add_y = -add_y  # from own perspective
    i1 = (i + add_x) % 8
    j1 = (j + add_y) % 8
    # check what's on target square
    # and manipulate position accordingly
    cb_position[i1][j1] = (B_PAWN, piece_nr)
    piece_list[piece_nr] = (i1, j1)
    m = chess.Move.from_uci(SQN[i][j] + SQN[i1][j1])
    print(SQN[i][j] + SQN[i1][j1])
    node = node.add_variation(m)
...
def decode(game_bytes, cb_position, w_piece_lists, b_piece_lists):
    (w_queens, w_rooks, w_bishops, w_knights, w_kings, w_pawns) = w_piece_lists
    (b_queens, b_rooks, b_bishops, b_knights, b_kings, b_pawns) = b_piece_lists
    position_stack = []
    processed_moves = 0
    game = chess.pgn.Game()
    node = game
    for idx in range(0, len(game_bytes)):
        ...
        if node.board().turn == chess.WHITE:
            ...
            if tkn == 0xAA: # null move
                ...
                node = do_piece_move(w_kings, W_KING, 0, cb_position, CB_KING_ENC, node, tkn)
            elif tkn in CB_QUEEN_1_ENC:
                node = do_piece_move(w_queens, W_QUEEN, 0, cb_position, CB_QUEEN_1_ENC, node, tkn)
            elif tkn in CB_QUEEN_2_ENC:
                node = do_piece_move(w_queens, W_QUEEN, 1, cb_position, CB_QUEEN_2_ENC, node, tkn)
            elif tkn in CB_QUEEN_3_ENC:
                node = do_piece_move(w_queens, W_QUEEN, 2, cb_position, CB_QUEEN_3_ENC, node, tkn)
            elif tkn in CB_ROOK_1_ENC:
                node = do_piece_move(w_rooks, W_ROOK, 0, cb_position, CB_ROOK_1_ENC, node, tkn)
            elif tkn in CB_ROOK_2_ENC:
                node = do_piece_move(w_rooks, W_ROOK, 1, cb_position, CB_ROOK_2_ENC, node, tkn)
            elif tkn in CB_ROOK_3_ENC:
                node = do_piece_move(w_rooks, W_ROOK, 2, cb_position, CB_ROOK_3_ENC, node, tkn)
            elif tkn in CB_BISHOP_1_ENC:
                node = do_piece_move(w_bishops, W_BISHOP, 0, cb_position, CB_BISHOP_1_ENC, node, tkn)
            elif tkn in CB_BISHOP_2_ENC:
                node = do_piece_move(w_bishops, W_BISHOP, 1, cb_position, CB_BISHOP_2_ENC, node, tkn)
            elif tkn in CB_BISHOP_3_ENC:
                node = do_piece_move(w_bishops, W_BISHOP, 2, cb_position, CB_BISHOP_3_ENC, node, tkn)
            elif tkn in CB_KNIGHT_1_ENC:
                node = do_piece_move(w_knights, W_KNIGHT, 0, cb_position, CB_KNIGHT_1_ENC, node, tkn)
            elif tkn in CB_KNIGHT_2_ENC:
                node = do_piece_move(w_knights, W_KNIGHT, 1, cb_position, CB_KNIGHT_2_ENC, node, tkn)
            elif tkn in CB_KNIGHT_3_ENC:
                node = do_piece_move(w_knights, W_KNIGHT, 2, cb_position, CB_KNIGHT_3_ENC, node, tkn)
            elif tkn in CB_PAWN_A_ENC:
                node = do_w_pawn_move(w_pawns, 0, cb_position, CB_PAWN_A_ENC, node, tkn)
            elif tkn in CB_PAWN_B_ENC:
                node = do_w_pawn_move(w_pawns, 1, cb_position, CB_PAWN_B_ENC, node, tkn)
            elif tkn in CB_PAWN_C_ENC:
                node = do_w_pawn_move(w_pawns, 2, cb_position, CB_PAWN_C_ENC, node, tkn)
            elif tkn in CB_PAWN_D_ENC:
                node = do_w_pawn_move(w_pawns, 3, cb_position, CB_PAWN_D_ENC, node, tkn)
            elif tkn in CB_PAWN_E_ENC:
                #print("pawn move e")
                node = do_w_pawn_move(w_pawns, 4, cb_position, CB_PAWN_E_ENC, node, tkn)
            elif tkn in CB_PAWN_F_ENC:
                node = do_w_pawn_move(w_pawns, 5, cb_position, CB_PAWN_F_ENC, node, tkn)
            elif tkn in CB_PAWN_G_ENC:
                node = do_w_pawn_move(w_pawns, 6, cb_position, CB_PAWN_G_ENC, node, tkn)
            elif tkn in CB_PAWN_H_ENC:
                node = do_w_pawn_move(w_pawns, 7, cb_position, CB_PAWN_H_ENC, node, tkn)
        else:
            #print("black to move")
            print("tkn: " + str(hex(tkn)))
            if tkn == 0xAA: # null move
                ...
                node = do_piece_move(b_kings, B_KING, 0, cb_position, CB_KING_ENC, node, tkn)
            elif tkn in CB_QUEEN_1_ENC:
                node = do_piece_move(b_queens, B_QUEEN, 0, cb_position, CB_QUEEN_1_ENC, node, tkn)
            elif tkn in CB_QUEEN_2_ENC:
                node = do_piece_move(b_queens, B_QUEEN, 1, cb_position, CB_QUEEN_2_ENC, node, tkn)
            elif tkn in CB_QUEEN_3_ENC:
                node = do_piece_move(b_queens, B_QUEEN, 2, cb_position, CB_QUEEN_3_ENC, node, tkn)
            elif tkn in CB_ROOK_1_ENC:
                node = do_piece_move(b_rooks, B_ROOK, 0, cb_position, CB_ROOK_1_ENC, node, tkn)
            elif tkn in CB_ROOK_2_ENC:
                node = do_piece_move(b_rooks, B_ROOK, 1, cb_position, CB_ROOK_2_ENC, node, tkn)
            elif tkn in CB_ROOK_3_ENC:
                node = do_piece_move(b_rooks, B_ROOK, 2, cb_position, CB_ROOK_3_ENC, node, tkn)
            elif tkn in CB_BISHOP_1_ENC:
                node = do_piece_move(b_bishops, B_BISHOP, 0, cb_position, CB_BISHOP_1_ENC, node, tkn)
            elif tkn in CB_BISHOP_2_ENC:
                node = do_piece_move(b_bishops, B_BISHOP, 1, cb_position, CB_BISHOP_2_ENC, node, tkn)
            elif tkn in CB_BISHOP_3_ENC:
                node = do_piece_move(b_bishops, B_BISHOP, 2, cb_position, CB_BISHOP_3_ENC, node, tkn)
            elif tkn in CB_KNIGHT_1_ENC:
                node = do_piece_move(b_knights, B_KNIGHT, 0, cb_position, CB_KNIGHT_1_ENC, node, tkn)
            elif tkn in CB_KNIGHT_2_ENC:
                node = do_piece_move(b_knights, B_KNIGHT, 1, cb_position, CB_KNIGHT_2_ENC, node, tkn)
            elif tkn in CB_KNIGHT_3_ENC:
                node = do_piece_move(b_knights, B_KNIGHT, 2, cb_position, CB_KNIGHT_3_ENC, node, tkn)
            elif tkn in CB_PAWN_A_ENC:
                node = do_b_pawn_move(b_pawns, 0, cb_position, CB_PAWN_A_ENC, node, tkn)
            elif tkn in CB_PAWN_B_ENC:
                node = do_b_pawn_move(b_pawns, 1, cb_position, CB_PAWN_B_ENC, node, tkn)
            elif tkn in CB_PAWN_C_ENC:
                node = do_b_pawn_move(b_pawns, 2, cb_position, CB_PAWN_C_ENC, node, tkn)
            elif tkn in CB_PAWN_D_ENC:
                node = do_b_pawn_move(b_pawns, 3, cb_position, CB_PAWN_D_ENC, node, tkn)
            elif tkn in CB_PAWN_E_ENC:
                #print("pawn move e black")
                node = do_b_pawn_move(b_pawns, 4, cb_position, CB_PAWN_E_ENC, node, tkn)
            elif tkn in CB_PAWN_F_ENC:
                node = do_b_pawn_move(b_pawns, 5, cb_position, CB_PAWN_F_ENC, node, tkn)
            elif tkn in CB_PAWN_G_ENC:
